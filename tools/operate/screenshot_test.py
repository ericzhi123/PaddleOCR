...
class ScreenArea(QtCore.QObject):
    ...
    def physicalPixmap(self, rectf, editAction=False):
        ...
        if editAction:
            ...
            '''_screenPixmap是物理分辨率的桌面截图，如1920*1080的桌面截图；
            将逻辑屏幕上的截图区域，包括起始点坐标和截图区域长宽都乘以放大倍数如1.25后，再在_screenPixmap上截取相关区域，获得物理屏幕分辨率的图片'''
    ...
    def setEndPoint(self, pointf, remake=False):
        self._pt_end = pointf
        if remake:
            # logger.info("QPointF end point y: {}；x: {}".format(self._pt_end.y(), self._pt_end.x()))
            self.remakeNightArea()
    ...
    def setCenterArea(self, start, end):
        ...
        # logger.info("QPointF start point y: {}；x: {}".format(self._pt_start.y(), self._pt_start.x()))
        self._pt_end = end
        self.remakeNightArea()
    ...
    def saveGraffitiAction(self):
        if self._pointfs:
            self._actions.append(
                ('graffiti', self.god.toolbar.curColor(), self.god.toolbar.curLineWidth(), self._pointfs.copy()))
            self._pointfs.clear()
            self.god.isDrawing = False
...
class ScreenShotWidget(QWidget):
    ...
    def start(self):
        ...
        logger.info(
            "screenPhysicalRectF: height:{}; width: {} ".format(self.screenArea.screenPhysicalRectF().toRect().height(),
                                                                self.screenArea.screenPhysicalRectF().toRect().width()))
    ...
    def save2Clipboard(self):
        ...
        if self.hasScreenShot:
            ...
            '''此处保存的图像是logicalRect*放大倍率后获得的物理设备分辨率图像'''
